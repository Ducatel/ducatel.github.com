...
public class Visu extends JFrame {
	...
	/**
	 * Bouton permettant de calculer les tables de routage
	 */
	public JButton calcul;
	
	/**
	 * Bouton permettant de retourner a la permiere interface 
	 * (choix par fichier ou par IHM )
	 */
	public JButton raz;
	
	/**
	 * Graph representant le reseau
	 */
	public DefaultGraph reseau;
	
	/**
	 * String contenant le resultat des tables de routage
	 */
	private String tableRoutage;
	...
	private void constructionIHM(){
		...
		calcul=new JButton("Calculer les tables de routage");
		calcul.addActionListener(new EcouteurBouton());
		panBas.add(calcul);
		
		raz=new JButton("Retour a l'état initial");
	...
	private void calculRoutage(){
		...
		for(Node n: routeur){
			...
			for(Node nOrdi:ordi){
				tableRoutage+="<tr>";
				tableRoutage+="<th style='color:blue;' border=1>"+nOrdi+"</th>";
